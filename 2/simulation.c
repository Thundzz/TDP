...
{
	MPI_Datatype type[4] = { MPI_INT , MPI_DOUBLE, MPI_DOUBLE, MPI_INT}; 
  	int blocklen[4] = {1, NB_PARTICLES, 2*NB_PARTICLES, NB_PARTICLES}; 
	MPI_Aint i1,i2 ; 
	MPI_Aint disp[6];
	...
	MPI_Get_address(&p->globId[0], &i2); disp[3] = i2-i1;

	MPI_Type_struct(4, blocklen, disp, type, MPI_PSET); 
	...
	for (int i = 0; i < NB_ITER; ++i)
	{
		...
		for (int j = 0; j < nb_processes; ++j)
		{
			if(j % 2 ==0 ){
				...
				/* Envoi du buffer de calcul actuel. */
				MPI_Send_init(comm_buf, 1, MPI_COMM_BUF,
							 next_proc, 3, MPI_COMM_WORLD, &send_req);

				/* Récéption du buffer de calcul suivant*/
				MPI_Recv_init(calc_buf, 1, MPI_CALC_BUF,
							  prev_proc, 3, MPI_COMM_WORLD, &recv_req);
